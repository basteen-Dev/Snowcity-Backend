...
The server should start on `https://snowcity-backend-zjlj.onrender.com0` (or the PORT specified in your `.env` file).

## 🧪 Testing the OTP Endpoint

### 1. Check Server Health
```bash
curl https://snowcity-backend-zjlj.onrender.com0/health
```
...
```bash
curl -X POST https://snowcity-backend-zjlj.onrender.com0/api/auth/otp/send \
...
### 3. Verify OTP
```bash
curl -X POST https://snowcity-backend-zjlj.onrender.com0/api/auth/otp/verify \
  -H "Content-Type: application/json" \
  -d '{
    "otp": "123456",
    "email": "test@example.com"
...
```javascript
// Send OTP
fetch('https://snowcity-backend-zjlj.onrender.com0/api/auth/otp/send', {
  method: 'POST',
  headers: {
...
axios.post('https://snowcity-backend-zjlj.onrender.com0/api/auth/otp/send', {
  email: 'test@example.com',
  phone: '+1234567890',
  name: 'Test User',
  channel: 'sms',
...
- Check that you're using the correct URL: `https://snowcity-backend-zjlj.onrender.com0/api/auth/otp/send`
