...
async function checkDatabase() {
  try {
    ...
    console.log('\n🎯 Checking Combos:');
    const combosResponse = await axios.get('http://localhost:4000/api/combos');
    ...
    const attractionsResponse = await axios.get('http://localhost:4000/api/attractions');
    ...
    if (combosResponse.data?.data?.length > 0) {
      ...
      try {
        const comboSlotsResponse = await axios.get(`http://localhost:4000/api/combos/${firstComboId}/slots?date=2025-11-29`);
    ...
    if (attractionsResponse.data?.data?.length > 0) {
      ...
      try {
        const attractionSlotsResponse = await axios.get(`http://localhost:4000/api/attractions/${firstAttractionId}/slots?date=2025-11-29`);
