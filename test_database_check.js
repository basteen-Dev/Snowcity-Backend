...
async function checkDatabase() {
  try {
    ...
    console.log('\n🎯 Checking Combos:');
    const combosResponse = await axios.get('https://snowcity-backend-zjlj.onrender.com0/api/combos');
    ...
    const attractionsResponse = await axios.get('https://snowcity-backend-zjlj.onrender.com0/api/attractions');
    ...
    if (combosResponse.data?.data?.length > 0) {
      ...
      try {
        const comboSlotsResponse = await axios.get(`https://snowcity-backend-zjlj.onrender.com0/api/combos/${firstComboId}/slots?date=2025-11-29`);
    ...
    if (attractionsResponse.data?.data?.length > 0) {
      ...
      try {
        const attractionSlotsResponse = await axios.get(`https://snowcity-backend-zjlj.onrender.com0/api/attractions/${firstAttractionId}/slots?date=2025-11-29`);
