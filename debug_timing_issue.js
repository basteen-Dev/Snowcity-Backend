...
async function testDatabaseConnection() {
  ...
  try {
    ...
    const response = await axios.get('http://localhost:4000/api/combos');
    ...
    if (response.data?.data?.length > 0) {
      ...
      const slotsResponse = await axios.get(`http://localhost:4000/api/combos/${comboId}/slots?date=2025-11-29`);
