...
exports.createSlot = async (req, res, next) => {
  // Slots are now generated dynamically, no need to create them
  try {
    res.status(400).json({ 
      error: 'Slots are generated dynamically. Use the listSlots endpoint to view available slots.',
      message: 'Slots are automatically available from 10:00 AM to 8:00 PM in 1-hour intervals.'
    });
  } catch (err) {
    next(err);
  }
...
exports.createSlotsBulk = async (req, res, next) => {
  // Bulk slot creation is no longer needed since slots are dynamic
  try {
    res.status(400).json({ 
      error: 'Bulk slot creation is no longer needed.',
      message: 'Slots are generated dynamically from 10:00 AM to 8:00 PM in 1-hour intervals.'
    });
  } catch (err) {
    next(err);
  }
